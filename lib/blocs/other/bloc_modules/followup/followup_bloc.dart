...
class FollowupBloc extends Bloc<FollowupEvents, FollowupStates> {
  ...
  Stream<FollowupStates> mapEventToState(FollowupEvents event) async* {
    ...
    if (event is ComplaintFollowupSaveRequestEvent) {
      yield* _mapComlaintFollowupEventToState(event);
    }

    if (event is ComplaintFollowupHistoryListRequestEvent) {
...
/*  Stream<FollowupStates> _mapFollowupDeleteCallEventToState(
  ...
  Stream<FollowupStates> _mapComlaintFollowupEventToState(
      ComplaintFollowupSaveRequestEvent event) async* {
    try {
      baseBloc.emit(ShowProgressIndicatorState(true));
      ...
      yield ComplaintFollowupSaveResponseState(event.context, response);
    } catch (error, stacktrace) {
      baseBloc.emit(ApiCallFailureState(error));
      print(stacktrace);
    } finally {
  ...
  Stream<FollowupStates> _mapComlaintFollowupHistoryListEventToState(
      ComplaintFollowupHistoryListRequestEvent event) async* {
    try {
      baseBloc.emit(ShowProgressIndicatorState(true));
      ...
      yield ComplaintFollowupHistoryListResponseState(response);
    } catch (error, stacktrace) {
      baseBloc.emit(ApiCallFailureState(error));
      print(stacktrace);
    } finally {
